...
public class SoulHealCommand implements CommandExecutor {
    
    private final SoloArisePlugin plugin;
    
    public SoulHealCommand(SoloArisePlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length < 1) {
            player.sendMessage("§cUsage: /soulheal <soulname>");
            return true;
        }
        ...
        if (soul == null) {
            player.sendMessage("§cSoul not found! Use /souls to see your souls.");
            return true;
        }
